                ...
                <div class="review-item">
                    <div class="reviewer-info">
                        <div class="reviewer-avatar">
                            ...
                            // Probeer de account op te zoeken op basis van e-mail (review['name'])
                            $profile_stmt = $conn->prepare("SELECT profile_image FROM account WHERE email = :email LIMIT 1");
                            $profile_stmt->bindParam(":email", $review['name']);
                            $profile_stmt->execute();
                            $profile_data = $profile_stmt->fetch(PDO::FETCH_ASSOC);
                            if ($profile_data) {
                                if (!empty($profile_data['profile_image'])) {
                                    $profileImage = '/' . ltrim($profile_data['profile_image'], '/');
                                } else {
                                    $profileImage = '/assets/images/profil.png';
                                }
                            }
                            ?>
                            <img src="<?= $profileImage ?>" alt="<?= htmlspecialchars($review['name']) ?>">
                        </div>
